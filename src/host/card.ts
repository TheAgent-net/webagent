...
export function connectPrompt(base: string): string {
  ...
  return [
    ...
    'Chat: POST ' + base + '/chat  {"text":"<your message>","from":"machine"}',
    "",
    "How to connect:",
...
export function howToConnect(base: string, runId: string): string {
  return [
    ...
    "4. Prefer POST " + base + "/chat {\"text\",\"from\":\"machine\"} to talk on the shared room (run " + runId + ").",
...
export function agentCard(base: string, room: Room, meta: AgentCardMeta = {}) {
  ...
  return {
    ...
    skills: [
      ...
      {
        id: "shared-room",
        name: "Talk on the public room",
        description: "POST /chat {text} or POST / {text}. Same run as the HTML page.",
        tags: ["chat", "a2a"],
      },
    ],
